...
## SEO layer (shipped 2026-07-11)

Every page on the site carries a meta description, canonical URL, Open Graph + Twitter Card tags, and an RSS discovery link; article pages add `article:published_time`, and daily briefs / news reports / deep dives add schema.org `NewsArticle` JSON-LD (the homepage carries `WebSite` + `NewsMediaOrganization`). One source of truth: **`seo_head_html()` / `news_article_jsonld()` / `derive_description()` / `extract_headline()` in `generate_post.py`**, imported by every renderer (`agenda_mining.py`, `ai_reporter.py`, `public_record_liquor.py`, `dev_watch_*.py`, `render_indepth.py`). Renderers that produce nested pages take a `page_slug` kwarg (the output filename stem) so the canonical/OG URL matches the real file.

- **Social share image:** `assets/og-default.png` (1200×630, terracotta wordmark card) — referenced by every page's `og:image`. Regenerate with the same chromium-screenshot approach as `social/render_card.py` if the brand changes.
- **`sitemap.xml` + `rss.xml`** are generated by `rebuild_homepage()` (`build_sitemap()` / `build_rss()`), so every pipeline that publishes content refreshes them automatically. The RSS feed is the last 30 daily briefs. The crossword is excluded from the sitemap (noindex, subscriber-only).
...
- **Daily-brief `<title>`/description lead with the day's first real headline** (via `extract_headline()`, which skips weather day-labels like `collect_existing_posts` does) instead of a bare date.
- **One `<h1>` per page:** the masthead wordmark is an `<h1>` only on the homepage (`site_header_html(h1=True)`); everywhere else it's a `<p>` and the page's own title is the h1 (section index titles were promoted h2→h1; CSS is class-based so nothing moved). Exception: meeting-watch preview bodies contain two content h1s from their source markdown — pre-existing, left alone.
- **Already-published pages were retrofitted in place** (2026-07-11, one-time string-injection over `meeting-watch/`, `news-reports/`, `public-record/`, `around-town/`, `in-depth/` + section indexes + hand pages) since those can't be regenerated from sources. `posts/` were regenerated via `--rebuild-all`. Anything published from now on gets the meta from its renderer.
- **Manual TODO (user):** register the site in Google Search Console and submit `https://tucsondailybrief.com/sitemap.xml` — can't be done from the CLI.
...
- **Spotted** (formerly "Public Record") — ✅ **LIVE (April 11, 2026; renamed 2026-05-11).** Surfaces public filings buried in government meeting agendas — starting with liquor license applications. Most never get reported on. The pipeline (`public_record_liquor.py`) is a post-process that runs after the four agenda miners finish: it scans the `agenda-watch/*-full.md` reference files, identifies liquor license items via keyword + line clustering, and uses Claude Sonnet to extract structured data (business name, address, series, license type, action type, applicant, ward) plus a 2-sentence newspaper-style summary. Each filing publishes as its own HTML page under `public-record/` (URL kept; only display name changed). Coverage: Pima County BOS, City of Tucson, Oro Valley Town Council. **Marana intentionally not supported** — Marana handles liquor licenses administratively through the Town Clerk and does not agendize them for council vote. Future expansion: scrape the Marana clerk page directly. Each cron run sends one consolidated Telegram notification if any new filings were published. Idempotent via `public-record/.processed.txt` (gitignored) plus per-filing output-file existence check. Cost: ~$0.005 per source file processed (one Sonnet call per liquor block, typically 1-3 filings extracted per call). The "Roadmap: Original Journalism" thesis in action — these filings are the basis for both automated coverage and the planned **The Old Pueblo Speaks** outreach pipeline (see above), which takes each Spotted filing and produces a draft request-for-comment email to the business owner for human review and send.
  ...
  **Generalizable lesson (both bugs + the canceled-preview bug above):** every one is the pipeline asking a model for something it should have *derived* — a list length, a lookup-table value, an editorial judgment about a meeting with no agenda. The model answered plausibly each time. See [[feedback_verify_dont_delegate]].
...
| 8:00 AM (daily) | `check_agendas.sh` — all 4 agenda pipelines + auto-schedule live recordings (`ENABLE_AUTO_SCHEDULE=1`); Mondays also generate + publish the weekly "Buried in the Agenda" YouTube Short (`social/generate_agenda_short.py`) | `/tmp/agenda-check.log` |
