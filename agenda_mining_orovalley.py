#!/usr/bin/env python3
"""
Oro Valley Town Council — Agenda Mining Pipeline
...
summary. Previews are auto-published by check_agendas.sh.

Usage:
    python agenda_mining_orovalley.py                  # Check current month for meetings
    python agenda_mining_orovalley.py --seq 1124       # Analyze a specific meeting
    python agenda_mining_orovalley.py --publish <file>  # Publish preview to site

Output saved to: agenda-watch/orovalley-YYYY-MM-DD-{preview,full}.md
"""
...
DESTINY_ID = "67682"  # Oro Valley's Destiny system ID
...
Your job: identify up to the 3-6 most newsworthy items and explain WHY they matter to Oro Valley and greater Tucson-area residents. Think like a beat reporter: what affects people's lives, money, safety, or rights? Write for a resident who has never read an agenda, not for an editor and not for other reporters.
...
- Balancing growth with preserving desert character and open space
- Oro Valley Marketplace and commercial development along Oracle Road corridor
- Town budget and public safety staffing levels
- Regional transportation coordination with Tucson metro (RTA)
- Parks, trails, and recreation — major quality-of-life priorities for residents
...
def generate_preview(meeting_date: datetime, meeting_type: str, analysis: str,
    ...
    lines = []
    lines.append(f"# Oro Valley Town Council — {'Meeting Canceled' if canceled else 'What to Watch'}")
    ...
    lines.append(f"*Source: [Town of Oro Valley Agendas](https://destinyhosted.com/agenda_publish.cfm?id={DESTINY_ID})*")
...
def generate_full_report(meeting_date: datetime, meeting_type: str, agenda_text: str) -> str:
    ...
    lines = []
    lines.append(f"# Oro Valley Town Council — Full Agenda Reference")
    lines.append(f"## {day_of_week}, {date_str}")
    lines.append(f"\n{meeting_type}")
    lines.append("")
    lines.append("---")
...
def publish_preview(preview_path: str) -> None:
    ...
    slug = f"orovalley-council-{date.strftime('%Y-%m-%d')}"

    title = "Oro Valley Town Council — What to Watch"
...
def main():
    parser = argparse.ArgumentParser(description="Oro Valley Town Council agenda mining pipeline")
    ...
    if args.seq:
        ...
        meeting_type = "Town Council Regular Session"
        date_slug = meeting_date.strftime("%Y-%m-%d")
        base = f"orovalley-{date_slug}"
        ...
        if not args.no_llm:
            ...
            if canceled:
                ...
                analysis = canceled_analysis_md("Oro Valley Town Council", meeting_date)
            else:
    ...
    print(f"Checking for Oro Valley meetings in {month}/{year}...")
    ...
    for m in council_meetings:
        ...
        if not args.no_llm:
            ...
            if canceled:
                print("  Meeting is CANCELED — writing stub preview, skipping Claude")
                analysis = canceled_analysis_md("Oro Valley Town Council", m["date"])
            else:
                print("  Running editorial analysis with Claude...")
