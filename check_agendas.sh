...
# --- Bluesky + Threads ledger-diff posters (SOCIAL-AUTOPOST.md Part 3) ---
# Post anything newly published since the last run; idempotent, non-fatal.
echo "Posting new pages to Bluesky..."
python3 "$SCRIPT_DIR/social/bluesky_poster.py" \
    || echo "WARNING: Bluesky poster failed (non-fatal)"
echo "Posting new pages to Threads..."
python3 "$SCRIPT_DIR/social/threads_poster.py" \
    || echo "WARNING: Threads poster failed (non-fatal)"
